<template>
    <div @load="LoadMessage" class="container-chat" style="background-image:url(assets/img/bg-img/test.png);background-size:inherit;">
                ...
                <!--  -->
                    <div class ="chat-section">
                        <div class="main-wrapper">
                            ...
                            <!--  -->
                            <form class="msg-tex">
                                <input type="text" name="msg" ref="msgField" id="msg" placeholder="Nhập nội dung..." autocomplete="off">
...
export default {
    ...
    methods: {
        btnClickSendMsg(e){
            ...
            const DataMessageUserAndText = { // Tạo dữ liệu để gửi sang server
                id: new Date().getTime(),
            ...
            this.socketInstance.emit("socketClientSendDataToServer", DataMessageUserAndText);

            this.$refs.msgField.value = ''; //Sửa nội dung rỗng cho trường chat

            if(refinputMsgField == '') //Kiểm tra có nhập nội dung hay không
                alert("Bạn chưa nhập nội dung")

            this.socketInstance.on("socketServerSendDataToClient", function(Data){
                console.log("Nhận nội dung từ máy khác: " + Data.text + " " + Data.user);
                this.dataChat = this.dataChat + "ccc" + Data.text;
                alert(Data.text)
            })
        },

        AddMessage(){ //Tạo phương thức nối nội dung
            const data = {
                id: new Date().getTime(),
                text: this.$refs.msgField.value,
                user:"Temp"
